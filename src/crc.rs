...
//! A common library for performing checksum calculations

/// A trait for checksum types like CRC-8 and CRC-16
pub trait Checksum: Copy + Clone + Default {
    /// Updates checksum with new byte
    fn update(self, byte: u8) -> Self;

    /// Returns OK if checksum is valid
    fn valid(self) -> bool;
}

/// A generic reader wrapper over Checksum types
pub struct CrcReader<R, C> {
    reader: R,
    checksum: C,
}
...
impl<R: std::io::Read, C: Checksum> std::io::Read for CrcReader<R, C> {
...
impl Checksum for Crc8 {
    fn update(self, byte: u8) -> Self {
        ...
        Self(SUMTABLE[(self.0 ^ byte) as usize])
    }
